...
export default function V2BudgetPanel({ repartoBudgets, totalBudget }) {
  const overallPct = totalBudget.budgetVol > 0
    ? (totalBudget.usedVol / totalBudget.budgetVol) * 100
    : 0;
    ...
    <aside className="w-80 shrink-0 border-r border-gray-100 bg-white flex flex-col overflow-hidden">
      ...
      <div className="p-5 border-b border-gray-100 bg-gradient-to-br from-dimar-dark via-slate-800 to-indigo-900 text-white">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] uppercase tracking-wider opacity-70 font-semibold">Totale Volantino</span>
          <span className="text-[10px] opacity-60">{Math.round(overallPct)}%</span>
        </div>
        <div className="flex items-baseline gap-1">
          <span className="text-3xl font-bold tabular-nums">{totalBudget.usedVol}</span>
          <span className="text-lg opacity-60">/ {totalBudget.budgetVol}</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-4 pt-3 border-t border-white/10">
          <div>
            <div className="text-[9px] uppercase opacity-60 tracking-wider">Affiancamento</div>
            <div className="text-sm font-semibold tabular-nums">
              {totalBudget.usedAff}<span className="opacity-50 text-xs">/{totalBudget.budgetAff}</span>
            </div>
          </div>
          <div>
            <div className="text-[9px] uppercase opacity-60 tracking-wider">Card</div>
            <div className="text-sm font-semibold tabular-nums">
              {totalBudget.usedCard}<span className="opacity-50 text-xs">/{totalBudget.budgetCard}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3">
        ...
        <div className="space-y-1">
          {repartoBudgets.map(r => {
            const color = budgetColor(r.usedVol, r.budget_vol);
            const textColors = {
            ...
            return (
              ...
              >
                <DonutRing used={r.usedVol} budget={r.budget_vol} />
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[10px] mt-0.5">
                    <span className={`font-mono font-bold ${textColors[color]} tabular-nums`}>
                      {r.usedVol}/{r.budget_vol}
                    </span>
                    {r.budget_aff > 0 && (
                      <span className="text-gray-400 font-mono tabular-nums">
                        · A {r.usedAff}/{r.budget_aff}
                      </span>
                    )}
                    {r.budget_card > 0 && (
                      <span className="text-gray-400 font-mono tabular-nums">
                        · C {r.usedCard}/{r.budget_card}
                      </span>
                    )}
                  </div>
                </div>
